...
アツマチは、全国のパチスロ店舗の来店イベント情報を自動収集・集約するモバイルアプリケーションです。10以上の情報ソースから最新のイベント情報を収集し、地図上での表示、検索、フィルタリング機能を提供します。AI（LLM）を活用した高精度なイベント情報抽出により、ユーザーに正確で最新の情報を届けます。

## 主な機能

- **来店イベント情報の自動収集**: 10以上のソースから最新のイベント情報をスクレイピング
...
│   ├── _core/                     # コア機能（サーバー起動等）
│   ├── scrapers/                  # スクレイピング機能
│   │   ├── drillermaguro.ts       # ドリラーマグロ
│   │   ├── raitenex.ts            # 来店EX
│   │   ├── hallnavi.ts            # ホールナビ
│   │   ├── hallnavi-llm.ts        # ホールナビ（LLM版）
│   │   ├── offme.ts               # オフミー
│   │   ├── touslo.ts              # 当スロ
│   │   ├── janbari.ts             # ジャンバリ
│   │   ├── hisshobon.ts           # 必勝本
│   │   ├── slopachistation.ts     # スロパチステーション
│   │   ├── dmmpachitown.ts        # DMMぱちタウン
│   │   ├── pworld.ts              # P-WORLD
│   │   ├── index.ts               # スクレイパー統合
...
このアプリは、以下の10以上のソースから自動的にイベント情報を収集します:

1. **ドリラーマグロ** (drillermaguro.com)
2. **来店EX** (raitenex)
3. **ホールナビ** (hallnavi) - LLM版も対応
4. **オフミー** (offme)
5. **当スロ** (touslo)
6. **ジャンバリ** (janbari)
7. **必勝本** (hisshobon)
8. **スロパチステーション** (slopachistation)
9. **DMMぱちタウン** (dmmpachitown)
10. **P-WORLD** (pworld)
