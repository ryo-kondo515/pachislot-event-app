...
| User accounts / login | Yes | Yes | OAuth Authentication |
...
2. `startOAuthLogin()` calls `Linking.openURL()` which opens OAuth in the system browser
...
When you need external data, check if there's any built-in API available. You only have to connect other APIs if there's no suitable built-in API available.
